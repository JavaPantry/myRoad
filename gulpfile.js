...
function buildTask() {
  ...
  var bundled = browserify('./src/RoadMap.js', { standalone: 'RoadMap' })
    .plugin(collapse)
    .bundle()
    .pipe(source('RoadMap.bundle.js'))
    ...
    .pipe(insert.prepend(header))
    .pipe(streamify(replace('{{ version }}', package.version)))
    .pipe(streamify(concat('RoadMap.bundle.min.js')))
    .pipe(gulp.dest(outDir));

  var nonBundled = browserify('./src/RoadMap.js', { standalone: 'RoadMap' })
    .ignore('moment')
    .plugin(collapse)
    .bundle()
    .pipe(source('RoadMap.js'))
    .pipe(insert.prepend(header))
    .pipe(streamify(replace('{{ version }}', package.version)))
    .pipe(gulp.dest(outDir))
    .pipe(streamify(uglify()))
    .pipe(insert.prepend(header))
    .pipe(streamify(replace('{{ version }}', package.version)))
    .pipe(streamify(concat('RoadMap.min.js')))
    .pipe(gulp.dest(outDir));
